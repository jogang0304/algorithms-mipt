...
int timer = 0;
std::vector<Edge> DfsGetBridges(const std::vector<std::vector<int>>& graph,
  ...
  for (auto next : graph[start]) {
    ...
    if (nodes[next].time_in == NoneValue) {
      auto subtree_answer = DfsGetBridges(graph, nodes, next, start);
...
void PrintAnswer(std::vector<Edge>& edges, std::vector<Edge>& bridges) {
  ...
  for (size_t i = 0; i < bridges.size(); ++i) {
    ...
    auto edge = std::lower_bound(edges.begin(), edges.end(), bridges[i]);
...
int main() {
  ...
  std::vector<Edge> bridges(0);
  for (int i = 0; i < amount_of_nodes; ++i) {
    if (nodes[i].time_in == NoneValue) {
      auto answer = DfsGetBridges(graph, nodes, i);
      for (auto element : answer) {
        bridges.push_back(element);
      }
    }
  }
  PrintAnswer(edges, bridges);
}